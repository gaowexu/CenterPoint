...
def class_agnostic_nms(box_scores, box_preds, nms_type, nms_thresh, nms_pre_max_size, nms_post_max_size, score_thresh=None):
    ...
    if box_scores.shape[0] > 0:
        box_scores_nms, indices = torch.topk(box_scores, k=min(nms_pre_max_size, box_scores.shape[0]))
        boxes_for_nms = box_preds[indices]
        keep_idx, selected_scores = getattr(iou3d_nms_utils, nms_type)(
                boxes_for_nms[:, 0:7], box_scores_nms, nms_thresh, pre_maxsize=nms_pre_max_size
        )
        selected = indices[keep_idx[:nms_post_max_size]]
